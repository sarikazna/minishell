...
/*   Updated: 2024/05/10 14:27:04 by srudman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_missing_env(t_shell *shell, char **envp)
{
	if (get_env_var(shell, "USER=") == NULL)
		shell->env[ft_strlen(shell->env)] = ft_strdup("USER="); // so just leave it empty or assign it a value?
	if (get_env_var(shell, "SHLVL=") == NULL)
		shell->env[ft_strlen(shell->env)] = ft_strdup("SHLVL=1");
	if (get_env_var(shell, "_=/usr/bin") == NULL)
		shell->env[ft_strlen(shell->env)] = ft_strdup("_=/usr/bin/env");
	if (get_env_var(shell, "OLDPWD=") == NULL)
		shell->env[ft_strlen(shell->env)] = ft_strjoin_modified("OLDPWD=", /* I need pwd here. I think I need to use getpwd() function*/);
	if (get_env_var(shell, "PWD=") == NULL)
		shell->env[ft_strlen(shell->env)] = ft_strjoin_modified("PWD=", /* I need pwd here. I think I need to use getpwd() function*/);
}
